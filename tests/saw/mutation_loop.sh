...
# 2. 对突变体 进行 SAW 验证，SAW验证时的输出放在 tmp 文件夹下
  ...
  echo "" > tmp/saw_log
  # 运行 SAW 验证程序
	sudo docker run -v $(pwd):/a ghcr.io/galoisinc/saw:nightly a/verify_cork_uncork.saw >> tmp/saw_log &
	sudo docker run -v $(pwd):/a ghcr.io/galoisinc/saw:nightly a/verify_drbg.saw >> tmp/saw_log &
	sudo docker run -v $(pwd):/a ghcr.io/galoisinc/saw:nightly a/verify_handshake.saw >> tmp/saw_log &
	sudo docker run -v $(pwd):/a ghcr.io/galoisinc/saw:nightly a/verify_HMAC.saw >> tmp/saw_log &
	sudo docker run -v $(pwd):/a ghcr.io/galoisinc/saw:nightly a/verify_imperative_cryptol_spec.saw >> tmp/saw_log &
	sudo docker run -v $(pwd):/a ghcr.io/galoisinc/saw:nightly a/verify_state_machine.saw >> tmp/saw_log &
  echo "Count is $count"
  count=$((count+1))
  # 等待SAW验证结束
  wait
